...
1. 打开左侧的“参考数据”。先新增支付渠道、币种和需要的汇率。
2. 打开“空间”，新增每一项订阅空间，填写成本、到期日和支付渠道。
3. 进入空间详情，添加母账号和子账号。出租账号会计入应收，自用账号只计数量。
4. 打开“设置”，按需要调整到期天数、提醒邮箱和邮件发送配置。
5. 回到“仪表盘”，优先处理续费工作台列出的空间与出租账号。
...
到“参考数据 -> 币种”和“参考数据 -> 汇率”补全 CNY 币种及汇率后，页面会显示相应的换算参考值。
